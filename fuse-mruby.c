...
#define MRB_FUSE_PRIVATE_DATA                                                  \
  ((struct mrb_fuse_private_data *)fuse_get_context()->private_data)
...
static int mrb_fuse_getattr(const char *path, struct stat *stbuf) {
  ...
  if (!mrb_fixnum_p(vmode)) {
    ...
    syslog(LOG_NOTICE, "path %s has invalid mode. %s", path,
           mrb_string_value_ptr(mrb, vstrinspect));
    res = -ENOENT;
    return res;
  }
  ...
  mrb_value vsize =
      mrb_hash_get(mrb, vh, mrb_symbol_value(mrb_intern_cstr(mrb, "size")));
  if (mrb_fixnum_p(vsize)) {
    stbuf->st_size = mrb_fixnum(vsize);
  }

  mrb_value vmtime =
      mrb_hash_get(mrb, vh, mrb_symbol_value(mrb_intern_cstr(mrb, "mtime")));
  MRB_FUSE_ERR_CHECK;
  ...
  /*
          ...
          mrb_value v = mrb_funcall(mrb, mrb_top_self(mrb), "rand", 1,
     mrb_fixnum_value(1024) );
...
static int mrb_fuse_create(const char *path, mode_t mode,
                           struct fuse_file_info *fi) {
  mrb_state *mrb = MRB_FUSE_PRIVATE_DATA->mrb;

  /* syslog(LOG_NOTICE, "%s:%d:private_data=%d", __FUNCTION__,__LINE__ ,
   * private_data ); */

  mrb_value vh = mrb_funcall(mrb, mrb_top_self(mrb), "create", 1,
                             mrb_str_new_cstr(mrb, path));
  MRB_FUSE_ERR_CHECK;
...
static int mrb_fuse_read(const char *path, char *buf, size_t size, off_t offset,
  ...
  syslog(LOG_NOTICE, "%s:%d:path=%s,size=%d,offset=%d,len=%d", __FUNCTION__,
         __LINE__, path, size, offset, len);
...
static int mrb_fuse_unlink(const char *path) {
  mrb_state *mrb = MRB_FUSE_PRIVATE_DATA->mrb;
  mrb_value vh = mrb_funcall(mrb, mrb_top_self(mrb), "unlink", 1,
                             mrb_str_new_cstr(mrb, path));
  MRB_FUSE_ERR_CHECK;
  return 0;
}
